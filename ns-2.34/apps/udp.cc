...
void UdpAgent::sendmsg(int nbytes, AppData* data, const char* flags)
{
	...
	while (n-- > 0) {
		...
		if (flags && (0 ==strcmp(flags, "NEW_BURST")))
			rh->flags() |= RTP_M;
		p->setdata(data);
#ifdef SMAC_YJ_MULTIHOP_UNICAST
...
//yanjun 07
static class MyNullAgentClass : public TclClass {
public:
	MyNullAgentClass() : TclClass("Agent/MyNull") {}
	TclObject* create(int, const char*const*) {
		return (new MyNullAgent());
	}
} class_mynull_agent;


void MyNullAgent::recv(Packet* p, Handler*)
{
...
MyNullAgent::MyNullAgent() : Agent(PT_UDP)
{
}
